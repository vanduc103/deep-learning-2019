...
def fc(inputs, num_out, name, activation_fn=None, biased=True):
    ...
    return tf.layers.dense(inputs=inputs, units=num_out, activation=activation_fn, 
                           kernel_initializer=w_init, use_bias=biased, name=name)
...
#################################################
    ...
    def _build_model(self):
        ...
        with tf.variable_scope("discriminator", reuse=self.reuse):
            ...
            net_txt = tf.tile(net_txt, [1, 16, 16, 1], name='d_txt/tile')
...
#################################################
    ...
    '''
    def __init__(self, input_seqs, batch_size, is_training=False, reuse=False):
        '''
    ...
    def _build_model(self):
        ...
        with tf.variable_scope("rnnencoder", reuse=self.reuse):
            ...
            # RNN encoder
            LSTMCell = tf.nn.rnn_cell.LSTMCell(self.t_dim, reuse=self.reuse)
